...
class Services {
	...
	public static function findChars($login) {
		...
		SELECT
			C.*, 
			S1.classid AS subclass1, 
			S2.classid AS subclass2, 
			S3.classid AS subclass3, 
			CLAN.clan_name, 
			CLAN.ally_name
		FROM
			characters AS C
		...
		LEFT JOIN
			character_subclasses AS S1 ON S1.obj_Id = C.obj_Id AND S1.class_index = '1'
		LEFT JOIN
			character_subclasses AS S2 ON S2.obj_Id = C.obj_Id AND S2.class_index = '2'
		LEFT JOIN
			character_subclasses AS S3 ON S3.obj_Id = C.obj_Id AND S3.class_index = '3'
		WHERE

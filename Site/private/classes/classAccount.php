...
class Account {
	...
	public static function Register($login, $pass, $accLvl, $email) {
		...
		$pass = str_replace("$2y$", "$2a$", password_hash($_POST["pass"], PASSWORD_BCRYPT));
		$sql = DB::Executa("INSERT INTO accounts (login, password, last_active, access_level, last_server, email, created_time) VALUES ('".$login."', '".$pass."', '0', '0', '0', '".$email."', '".time()."')");
		return $sql;
	...
	public static function updatePass($pass, $login) {
		...
		$pass = str_replace("$2y$", "$2a$", password_hash($_POST["newpass"], PASSWORD_BCRYPT));

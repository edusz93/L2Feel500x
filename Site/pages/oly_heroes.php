...
<form method="get" action="">
    <input type="hidden" name="page" value="oly_heroes">
...
if((intval($dpage['olyrak'])+intval($dpage['olyher'])+intval($dpage['olyall'])) > 1) {
    ...
    <div class='horMenu'>
        ...
        ".($dpage['olyher'] == 1 ? "<a href='?page=oly_heroes' class='act'>".$LANG[12999]."</a>" : "")."
...
<table cellspacing="0" cellpadding="0" border="0" class='default'>
    ...
    if(count($line) > 0) {
        for($i=0, $c=count($line); $i < $c; $i++) {
            // Aplica o filtro de clã
            $lineClan = (string)$line[$i]->clan;
            if ($clanFilter && $lineClan != $clanFilter) {
                continue;
            }

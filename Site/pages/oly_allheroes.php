...
<form method="get" action="">
    <input type="hidden" name="page" value="oly_allheroes">
    ...
    <select name="classFilter" id="classFilter">
        ...
        <?php
        // Gerar opções para a classe
        $classes = array(88 => 'Duelist', 89 => 'Dreadnought', 90 => 'Phoenix Knight', 91 => 'Hell Knight', 92 => 'Sagittarius', 93 => 'Adventurer', 94 => 'Archmage', 95 => 'Soultaker', 96 => 'Arcana Lord', 97 => 'Cardinal', 98 => 'Hierophant', 99 => 'Eva Templar', 100 => 'Sword Muse', 101 => 'Wind Rider', 102 => 'Moonlight Sentinel', 103 => 'Mystic Muse', 104 => 'Elemental Master', 105 => 'Eva Saint', 106 => 'Shillien Templar', 107 => 'Spectral Dancer', 108 => 'Ghost Hunter', 109 => 'Ghost Sentinel', 110 => 'Storm Screamer', 111 => 'Spectral Master', 112 => 'Shillien Saint', 113 => 'Titan', 114 => 'Grand Khauatari', 115 => 'Dominator', 116 => 'Doomcryer', 117 => 'Fortune Seeker', 118 => 'Maestro');
        foreach ($classes as $id => $name) {
            $selected = (isset($_GET['classFilter']) && $_GET['classFilter'] == $name) ? 'selected' : '';
...
if((intval($dpage['olyrak'])+intval($dpage['olyher'])+intval($dpage['olyall'])) > 1) {
    echo "
    <div class='horMenu'>
        ".($dpage['olyrak'] == 1 ? "<a href='?page=oly_rank'>Ranking</a>" : "")."
        ".($dpage['olyher'] == 1 ? "<a href='?page=oly_heroes'>".$LANG[12999]."</a>" : "")."
        ".($dpage['olyall'] == 1 ? "<a href='?page=oly_allheroes' class='act'>".$LANG[12025]."</a>" : "")."
    </div>
    ";
}
?>
...
<table cellspacing="0" cellpadding="0" border="0" class='default'>
    ...
    if (count($line) > 0) {
        for ($i = 0, $c = count($line); $i < $c; $i++) {
            // Aplica o filtro de clã e classe
            $lineClan = (string)$line[$i]->clan;
            $lineClass = (string)$line[$i]->class;
            if (($clanFilter && $lineClan != $clanFilter) || ($classFilter && $lineClass != $classFilter)) {
                continue;
            }
        ...
        // Exibir classes se nenhum dado for encontrado
        $i = 88;
        while (118 >= $i) {
            echo "
            <tr" . (($i % 2 == 0) ? " class='two'" : "") . ">
